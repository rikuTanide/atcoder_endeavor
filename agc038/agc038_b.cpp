...
int main() {
    ...
    auto fmi = [](int i, int j) { return min(i, j); };
    auto fma = [](int i, int j) { return max(i, j); };
    SegmentTree<int, decltype(fmi)> minTree(fmi, INT_MAX);
    SegmentTree<int, decltype(fma)> maxTree(fma, 0);
    minTree.build(v);
    maxTree.build(v);
    ...
    rep(i, n - k) {
        ...
        int mi = minTree.query(l, r + 1);
        int ma = maxTree.query(l, r + 1);

        int out = v[i];
        int in = v[j];

        if (out < mi && ma < in) continue;
        else ans++;
