...
int main() {
    ...
    rep(i, n) {
        auto rc = range_count(v.begin(), v.end(), v[i], v[i] + d);
        right[i] = rc.between + rc.r_equal;
    }

    rep(i, n) {
        auto rc = range_count(v.begin(), v.end(), v[i] - d, v[i]);
        left[i] = rc.between + rc.l_equal;
    }

    ll ans = 0;
    rep(i, n) {
        ll now = left[i] * right[i];
        ans += now;
    }
